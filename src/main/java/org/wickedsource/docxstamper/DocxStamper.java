...
public class DocxStamper<T> {
    ...
    private void initFields() {
        ...
        typeResolverRegistry.registerTypeResolver(Date.class, new DateResolver("yyyy.MM.dd"));
    ...
    public void stamp(WordprocessingMLPackage document, T contextRoot, OutputStream out) throws DocxStamperException {
        try {
            ...
            endAction(document, proxyBuilder);
            document.save(out);
    ...
    private void endAction(final WordprocessingMLPackage document, ProxyBuilder<T> proxyBuilder) {
        placeholderReplacer.endProcessors(document, proxyBuilder);
    }
